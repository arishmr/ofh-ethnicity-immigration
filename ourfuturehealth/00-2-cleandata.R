...
## DROP PARTICIPANTS IF THEY HAVE DEPRESSION MISSING
...
## Recode the following categorical variables: alcohol, smoking (logical), social (degrees, not binary)
...
alldata <- alldata %>% mutate(smoking = case_when(
  is.na(orig_smoking_type) ~ NA,
  orig_smoking_type == "Prefer not to answer" ~ NA,
  orig_smoking_100 == "Prefer not to answer" ~ NA,
  orig_smoking_100 == "100 or more" ~ TRUE,
  TRUE ~ FALSE
))
...
alldata <- alldata %>% relocate(
  pid, #participant ID
  depression, currdep, undiagdep, PHQ, #outcomes

#####################################
## GOAL: within each immigration and ethnicity sub-group, calculate Odds Ratios for depression (lifetime diagnosis / PHQ-9 ≥ 10)
#####################################
...
# (2) run 2x logistic regression models, and
...
logreg_ethnicity <- function (data, subset, outVar) {
  ...
  fit1 <- glm(as.formula(paste(outVar, "~ ethnicity + age + sex + bmi + income + alcohol + smoking + loneliness")),
              family = binomial(link="logit"),
              data = df)
  ## extract relevant estimates from the model
  estimates <- data.frame(
...
output <- logreg_ethnicity(data, "Overall", "depression")
...
write.csv(output, "03-results/logreg_dep_ethnicity.csv", row.names = FALSE)
...
logreg_immigration <- function (data, subset, outVar) {
  ...
  # Fully adjusted model
  fit1 <- glm(as.formula(paste(outVar, "~ immigration + age + sex + bmi + income + alcohol + smoking + loneliness")),
              family = binomial(link="logit"),
              data = df)
  ## extract relevant estimates from the model
  estimates <- data.frame(
...
output <- logreg_immigration(data, "Hispanic or Latinx", "depression")
output <- logreg_immigration(data, "MENA", "depression")
...
output <- logreg_immigration(data, "Hispanic or Latinx", "currdep")
output <- logreg_immigration(data, "MENA", "currdep")
...
## Export output dataframe
write.csv(output, "03-results/logreg_dep_immigration.csv", row.names = FALSE)
